...
{
    delegate BinaryExpression LogicalExpression(Expression left, Expression right);
    ...
    internal sealed class FilteringService<TView> : RqlService, IFilteringService<TView>
    {
        ...
        public FilteringService(IOperatorHandlerProvider operatorHandlerProvider,
            ITypeMetadataProvider typeNameMaper,
            IRqlParser parser) : base(typeNameMaper)
        {
        ...
        private ErrorOr<Expression> MakeGroupExpression(ParameterExpression pe, RqlGroup group)
        {
            ...
            {
                RqlAnd and => (ErrorOr<LogicalExpression>)Expression.AndAlso,
                RqlOr or => (ErrorOr<LogicalExpression>)Expression.OrElse,
        ...
        private ErrorOr<Expression> MakeBinaryExpression(ParameterExpression pe, RqlBinary node)
        {
            ...
            if (expression.IsError)
                return AssignErrorCode(expression.Errors, MakeErrorCode(memberConstant.Value));

            return expression;

            static List<Error> AssignErrorCode(List<Error> errors, string code)
        ...
        private Error MakeInternalError() => Error.Failure(MakeErrorCode("internal"), "Internal filtering error occured. Please contact RQL package maintainer.");
    }
}
