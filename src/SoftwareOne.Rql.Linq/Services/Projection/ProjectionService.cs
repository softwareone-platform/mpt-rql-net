...
internal delegate ErrorOr<Expression?> ComplexPropertyProcessor(MemberExpression member, ProjectionNode node, int depth);
...
internal class ProjectionService<TView> : RqlService, IProjectionService<TView>
{
    ...
    public ProjectionService(ITypeMetadataProvider typeMetadataProvider, IRqlParser parser) : base(typeMetadataProvider)
    {
    ...
    private ErrorOr<MemberInitExpression> GetSelector(Expression param, ProjectionNode node, int depth)
    {
        ...
        var properties = _typeMetadataProvider.ListProperties(param.Type);
        ...
        foreach (var item in properties)
        {
            ...
            else
            {
                ...
                if (!item.Value.Flags.HasFlag(MemberFlag.IsDefault))
                    continue;
            }
